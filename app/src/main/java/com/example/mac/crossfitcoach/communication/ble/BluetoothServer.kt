...
class BluetoothServer(context: Context, val onDeviceConnectedCallback: OnDeviceConnectedCallback) {
        ...
        override fun onConnectionStateChange(device: BluetoothDevice?, status: Int, newState: Int) {
            ...
            if (newState == BluetoothProfile.STATE_CONNECTED) {
                ...
                if (!connectedDevices.contains(device)) {
                    ...
                    onDeviceConnectedCallback.onDeviceConnected(device!!)
                }
                stopAdvertising()
            } else if (newState == BluetoothProfile.STATE_DISCONNECTED) {
                Log.d("Andrea", "connection disconnected on server")
                onDeviceConnectedCallback.onDeviceDisconnected(device!!)
    ...
    interface OnDeviceConnectedCallback {
